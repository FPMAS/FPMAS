...
void SyncMpiCommunicator::handleIncomingRequests() {
	...
	if(flag > 0) {
		MpiDistributedId id;
		MPI_Recv(&id, 1, MPI_DISTRIBUTED_ID_TYPE, req_status.MPI_SOURCE, epoch | Tag::READ, this->getMpiComm(), &req_status);
		FPMAS_LOGD(this->getRank(), "RECV", "read request %lu from %i", id, req_status.MPI_SOURCE);
		this->handleRead(req_status.MPI_SOURCE, DistributedId(id));
	}
	...
	if(flag > 0) {
		MpiDistributedId id;
		MPI_Recv(&id, 1, MPI_DISTRIBUTED_ID_TYPE, req_status.MPI_SOURCE, epoch | Tag::ACQUIRE, this->getMpiComm(), &req_status);
		FPMAS_LOGD(this->getRank(), "RECV", "acquire request %lu from %i", id, req_status.MPI_SOURCE);
		this->handleAcquire(req_status.MPI_SOURCE, DistributedId(id));
	}
 ...
 */
std::string SyncMpiCommunicator::read(DistributedId id, int location) {
	...
	MPI_Request req;
	MPI_Issend(&id, 1, MPI_DISTRIBUTED_ID_TYPE, location, epoch | Tag::READ, this->getMpiComm(), &req);
 ...
 * queue otherwise.
 */
void SyncMpiCommunicator::handleRead(int destination, DistributedId id) {
 ...
 * resourceManager.
 */
void SyncMpiCommunicator::respondToRead(int destination, DistributedId id) {
 ...
 */
std::string SyncMpiCommunicator::acquire(DistributedId id, int location) {
	if(location == this->getRank()) {
	...
	// Starts non-blocking synchronous send
	MPI_Request req;
	MPI_Issend(&id, 1, MPI_DISTRIBUTED_ID_TYPE, location, epoch | Tag::ACQUIRE, this->getMpiComm(), &req);
 ...
 * queue otherwise.
 */
void SyncMpiCommunicator::handleAcquire(int destination, DistributedId id) {
	this->color = Color::BLACK;
 ...
 * resourceManager.
 */
void SyncMpiCommunicator::respondToAcquire(int destination, DistributedId id) {
	std::string data = this->resourceContainer.getLocalData(id);
 ...
 * @param location rank of the data location
 */
void SyncMpiCommunicator::giveBack(DistributedId id, int location) {
	if(location == this->getRank()) {
