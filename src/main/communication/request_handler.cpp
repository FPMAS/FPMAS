...
namespace FPMAS::communication {
	 ...
	 */
	RequestHandler::RequestHandler(mpi_communicator& comm, ResourceHandler& resourceHandler)
		: resourceManager(*this), comm(comm), resourceHandler(resourceHandler) {
		};
	 ...
	 * - given back data
	 */
	void RequestHandler::handleIncomingRequests() {
	 ...
	 */
	void RequestHandler::waitSendRequest(MPI_Request* req) {
	 ...
	 */
	std::string RequestHandler::read(DistributedId id, int location) {
	 ...
	 * queue otherwise.
	 */
	void RequestHandler::handleRead(int destination, DistributedId id) {
	 ...
	 * resourceManager.
	 */
	void RequestHandler::respondToRead(int destination, DistributedId id) {
	 ...
	 */
	std::string RequestHandler::acquire(DistributedId id, int location) {
		if(location == this->comm.getRank()) {
	 ...
	 * queue otherwise.
	 */
	void RequestHandler::handleAcquire(int destination, DistributedId id) {
		this->color = Color::BLACK;
	 ...
	 * resourceManager.
	 */
	void RequestHandler::respondToAcquire(int destination, DistributedId id) {
		std::string data = this->resourceHandler.getLocalData(id);
	 ...
	 * @param location rank of the data location
	 */
	void RequestHandler::giveBack(DistributedId id, int location) {
		if(location == this->comm.getRank()) {
	 ...
	 */
	void RequestHandler::handleGiveBack(std::string data) {
	...
	const typename RequestHandler::mpi_communicator& RequestHandler::getMpiComm() {
		return this->comm;
	}
