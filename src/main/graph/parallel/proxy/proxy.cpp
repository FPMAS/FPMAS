...
namespace FPMAS::graph::parallel::proxy {
	 ...
	 */
	void Proxy::setCurrentLocation(DistributedId id, int proc) {
	 ...
	 * @param id node id
	 */
	int Proxy::getCurrentLocation(DistributedId id) const {
	 ...
	 * for this node
	 */
	void Proxy::setLocal(DistributedId id, bool upToDate) {
		if(!upToDate)
	...
	/**
	 * Returns true iff data corresponding to `id` is located on the current
	 * proc.
	 *
	 * @return true iff data is local
	 */
	bool Proxy::isLocal(DistributedId id) {
	...
	void obj_size_multi_fn(
		...
		for (int i = 0; i < num_ids; ++i) {
			DistributedId nodeId = read_zoltan_id(&global_ids[i * num_gid_entries]);
	...
	void pack_obj_multi_fn(
		...
		for (int i = 0; i < num_ids; ++i) {
			// Rebuilt node id
			DistributedId id = read_zoltan_id(&global_ids[i * num_gid_entries]);
