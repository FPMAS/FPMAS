...
TEST_F(Mpi_ZoltanFunctionsTest, obj_size_multi_test) {
	...
	write_zoltan_node_id(0, &global_ids[0]);
	write_zoltan_node_id(85250, &global_ids[2]);
...
TEST_F(Mpi_ZoltanFunctionsTest, pack_obj_multi_test) {
	...
	// Computes required buffer sizes
	// Also implicitly writes json string to internal cache
	int dest[2]; // dest, unused
	int sizes[2];
