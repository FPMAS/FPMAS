...
TEST_F(Mpi_OlzTest, simpleGhostNodeTest) {
	...
	ASSERT_EQ(dg.getNode(1ul)->getOutgoingArcs().size(), 2);
	// Nothing has changed for node 2
	ASSERT_EQ(dg.getNode(2ul)->getIncomingArcs().size(), 2);
	...
	ASSERT_EQ(dg.getNode(1ul)->getOutgoingArcs().size(), 1);
	...
	ASSERT_EQ(
			*(dg.getNode(1ul)->getOutgoingArcs().at(0)->getTargetNode()->getData()),
			2
			);
	ASSERT_EQ(
			dg.getNode(1ul)->getOutgoingArcs().at(0)->getTargetNode()->getId(),
			2ul
			);
