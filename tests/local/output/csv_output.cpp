...
TEST(CsvOutputBase, job) {
	struct MockCsvOuput : public fpmas::output::CsvOutputBase<int, float> {
		using fpmas::output::CsvOutputBase<int, float>::CsvOutputBase;

		MockCsvOuput(std::ostream& o,
				std::pair<std::string, std::function<int()>> f,
				std::pair<std::string, std::function<float()>> g
				)
			: fpmas::output::CsvOutputBase<int, float>(o, f, g) {}
...
TEST(CsvOutput, output) {
	...
	fpmas::output::CsvOutput<int, std::string> csv_out {
		out,
		{"field_1", [&i] () {return i;}},
		{"field_2", [&s] () {return s;}}
	};
